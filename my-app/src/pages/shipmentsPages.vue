      ...
      <div v-if="orderList.length > 0">
        <div class="listBox" v-for="(citem, cindex) in orderList" :key="cindex">
          <div class="detail_box">
            ...
            <div class="detail_text">
              <p align="right">提交时间：</p>
              <p>{{ citem.OrderDate }}</p>
            </div>
            <div class="detail_text">
              <p align="right">订单总计：</p>
              <p>
                {{
                  citem.RowNumber +
                    "件" +
            ...
            <div class="detail_text">
              <p align="right">备注：</p>
              <p>{{ citem.ManagerRemark }}</p>
            </div>
          </div>

          <!-- <div class="list_title">
            <p>订单号：{{ citem.OrderId }}</p>
            <p style="margin-left: 10px;">会员名称：{{ citem.Username }}</p>
          </div>
          ...
          <div class="detail_box">
            ...
            <div class="detail_text">
              <p align="left">提交时间:{{ citem.OrderDate }}</p>
              <p align="left">
                订单总计:{{
                  citem.RowNumber +
                    "件" +
              ...
              <p align="left">备注:{{ citem.ManagerRemark }}</p>
            </div>
          </div> -->
          ...
          <div class="footer_box">
            <van-row type="flex" justify="center" align="center">
              ...
              <van-col span="14" type="flex" justify="space-between">
                ...
                > -->
                <van-button
                  style="color: white;background-color:#00437C ;margin-left: 5px;width: 100px;"
                  size="small"
                  @click="handleShipments(citem)"
                  >发货</van-button
                >
              </van-col>
            </van-row>
...
export default {
  ...
  methods: {
    ...
    async handleOnload(paramsObj = {}) {
      ...
      const url = `https://www.daogoujingling.com/api/wechatApplet.ashx?customId=1079&action=GetManagerOrderList`;
    ...
    handleDateRange() {
      ...
      this.page = {
        ...
        current: 1,
        pageSize: 5,
        StartDate: this.dateScope[0],
        EndDate: this.dateScope[1]
      };
    ...
    handleShipments(dataObj) {
      ...
      // this.$router.push({
      //   path: `shipmentsPagesTow?true&OrderId=${dataObj.OrderId}&workerid=${workerid}`
      // });
    },
